...
namespace Statistics.Test
{
    public class StatsUnitTest
    {
        ...
        public void ReportsAverageMinMax()
        {
            ...
            Assert.True(Math.Abs(statsComputer.getAvg() - 4.525) <= epsilon);
            Assert.True(Math.Abs(statsComputer.getMax() - 8.9) <= epsilon);
            Assert.True(Math.Abs(statsComputer.getMin() - 1.5) <= epsilon);
        }
        [Fact]
